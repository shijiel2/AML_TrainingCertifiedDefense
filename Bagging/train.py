...
def main():
    ...
    for repeat_time in range(int(args.start),int(args.end)):
        ...
        startsampling_time = timeit.default_timer()
        ...
        endsample_time = timeit.default_timer()
        print('subsample size:', len(x_train_sample))
        print('sampling time:', endsample_time - startsampling_time)
        ...
        # starteval_time = timeit.default_timer()
        # score = model.evaluate(x_test, y_test, verbose=0)
        # endeval_time = timeit.default_timer()
        # print('Test loss:', score[0])
        # print('Test accuracy:', score[1])
        # print('eval time:', endeval_time - starteval_time)

        print('test size:', len(x_test))
        startpred_time = timeit.default_timer()
        prediction_label = np.argmax(model.predict(x_test),axis=1)
        aggregate_result[np.arange(0,x_test.shape[0]),prediction_label] += 1
        endpred_time = timeit.default_timer()
        print('preding time:', endpred_time - startpred_time)
